...
## 2024-07-23 - Hardcoded Secrets in Launch Scripts
**Vulnerability:** Found hardcoded Moonshot API keys (`MOONSHOT_API_KEY`) acting as fallbacks in multiple launcher scripts (`launcher.bat`, `launch.cjs`, `start-vent.bat`).
**Learning:** Convenience fallbacks in launcher scripts often lead to accidental commitment of live API keys into version control. Scripts should read configuration from environment variables without relying on hardcoded defaults.
**Prevention:** Launcher and setup scripts must enforce the presence of required environment variables and fail cleanly with an informative message if they are missing, rather than attempting to supply a default secret.
