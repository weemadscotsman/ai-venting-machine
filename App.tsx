  ...
  useEffect(() => {
    const decayInterval = setInterval(() => {
        if (machineState !== MachineState.PLAYING_SESSION && machineState !== MachineState.GENERATING_SCRIPT) {
            setAgents(prev => {
                // ⚡ BOLT OPTIMIZATION:
                // Prevents unnecessary React re-renders and localStorage disk I/O (from the useEffect dependent on `agents`)
                // by returning the exact previous array reference if no agent states were actually modified during this interval tick.
                ...
                const next = prev.map(a => {
                    ...
                    if (a.stressLevel !== newStress || a.status !== newStatus) {
                        hasChanges = true;
