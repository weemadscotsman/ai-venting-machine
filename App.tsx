            ...
            setAgents(prev => {
                ...
                const newAgents = prev.map(a => {
                    ...
                    if (a.stressLevel !== newStress || a.status !== newStatus) {
                        hasChanges = true;
                        // ⚡ Bolt: Return new object only if stress or status changed
                        return { ...a, stressLevel: newStress, status: newStatus as any };
                    }
                    return a;
                });
                // ⚡ Bolt: Prevent unnecessary re-renders and localStorage writes by returning the previous array reference if nothing changed
                return hasChanges ? newAgents : prev;
            });
        }
    }, 3000);
