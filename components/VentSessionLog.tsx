...
const MessageItem = React.memo(({
  ...
  return (
    <div className={`flex ${isRight ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-500 fade-in group`}>
...
export const VentSessionLog: React.FC<VentSessionLogProps> = ({ messages, agents, isPlaying, onSpeakerChange, apiKey }) => {
  ...
  // Latest ref pattern to avoid stale closures in handlePlayAudio while keeping stable reference
  ...
  const handlePlayAudio = useCallback(async (msg: VentMessage, voiceName?: string) => {
