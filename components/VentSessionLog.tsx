...
// ⚡ Bolt: Extract message item into a memoized component to prevent unnecessary re-renders of the entire list
...
const VentMessageItem = React.memo(({ msg, agent, idx, isPlayingAudio, isLoadingAudio, onPlayAudio }: VentMessageItemProps) => {
      ...
      <div className={`max-w-[85%] md:max-w-[75%] border border-gray-800 p-3 rounded-sm relative ${isRight ? 'bg-gray-900/50' : 'bg-black/80'}`}>
         <div className="flex items-center gap-2 mb-2 border-b border-gray-800 pb-1">
           ...
           <button
              ...
              className={`ml-2 p-1 rounded-full border border-gray-700 hover:bg-gray-800 transition-colors ${isPlayingAudio ? 'text-green-400 border-green-900 animate-pulse' : 'text-gray-500'}`}
              title="Play Voice"
              type="button"
...
// displayName helps with React DevTools profiling
VentMessageItem.displayName = 'VentMessageItem';
...
export const VentSessionLog: React.FC<VentSessionLogProps> = ({ messages, agents, isPlaying, onSpeakerChange, apiKey }) => {
  ...
  // ⚡ Bolt: Refs for latest state to avoid stale closures in handlePlayAudio
  ...
  // ⚡ Bolt: Memoized play function to pass to children stably
  ...
  }, []); // ⚡ Bolt: No dependencies needed due to refs, avoiding child re-renders
